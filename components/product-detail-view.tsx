...
// Dados fictícios para atributos
const atributos = [
  { value: "cor", label: "Cor" },
...
const formSchema = z.object({
  ...
  ecommerce: z.object({
    ...
    tempoPreparacao: z.string().optional(),
...
const produtoExemplo = {
  id: "1",
  foto: "/computer-monitor-display.png",
  nome: 'Monitor UltraWide 29"',
  codigo: "MON-UW29",
  descricao:
    "Monitor ultrawide com resolução de 2560x1080, painel IPS e taxa de atualização de 75Hz. Ideal para produtividade e multitarefas.",
  ncm: "8528.52.00",
  gtin: "7898590239876",
  codigoBarras: "7898590239876",
  categorias: ["eletronicos", "perifericos", "monitores"],
  temVariacoes: true,
  atributos: [
    {
      nome: "cor",
      valores: ["preto", "branco"],
    },
    {
      nome: "tamanho",
      valores: ["29", "34"],
  ...
  variacoes: [
    { nome: "Preto / 29", codigo: "MON-UW29-P", estoque: 15 },
    { nome: "Branco / 29", codigo: "MON-UW29-B", estoque: 8 },
    { nome: "Preto / 34", codigo: "MON-UW34-P", estoque: 5 },
    { nome: "Branco / 34", codigo: "MON-UW34-B", estoque: 3 },
  ],
  tabelasPreco: [
    { id: "1", nome: "Tabela Padrão", precoCusto: "R$ 1.200,00", precoVenda: "R$ 1.599,90" },
    { id: "2", nome: "Tabela Atacado", precoCusto: "R$ 1.200,00", precoVenda: "R$ 1.499,90" },
  ],
  ecommerce: {
    ativo: true,
    destaque: true,
    ...
    comprimento: "20",
    tempoPreparacao: "2",
    metaTitle: "Monitor UltraWide 29 polegadas | Loja Exemplo",
    metaDescription:
      "Monitor ultrawide com resolução de 2560x1080, painel IPS e taxa de atualização de 75Hz. Compre agora!",
    keywords: "monitor, ultrawide, 29 polegadas, IPS, 75Hz",
  },
}

export function ProductDetailView({ productId }: { productId: string }) {
  const [isEditing, setIsEditing] = React.useState(false)
  const [selectedCategories, setSelectedCategories] = React.useState<string[]>(produtoExemplo.categorias || [])
  ...
  const [atributosAdicionados, setAtributosAdicionados] = React.useState<Array<{ nome: string; valores: string[] }>>(
    produtoExemplo.atributos || [],
  )
  const [variacoes, setVariacoes] = React.useState<Array<{ nome: string; codigo: string; estoque?: number }>>(
    produtoExemplo.variacoes || [],
  )
  ...
  const [imageSrc, setImageSrc] = React.useState<string | null>(produtoExemplo.foto || null)
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      foto: produtoExemplo.foto || "",
      nome: produtoExemplo.nome || "",
      codigo: produtoExemplo.codigo || "",
      descricao: produtoExemplo.descricao || "",
      ncm: produtoExemplo.ncm || "",
      gtin: produtoExemplo.gtin || "",
      codigoBarras: produtoExemplo.codigoBarras || "",
      categorias: produtoExemplo.categorias || [],
      temVariacoes: produtoExemplo.temVariacoes || false,
      atributos: produtoExemplo.atributos || [],
      variacoes: produtoExemplo.variacoes || [],
      tabelasPreco: produtoExemplo.tabelasPreco || [],
      ecommerce: {
        ativo: produtoExemplo.ecommerce?.ativo || true,
        destaque: produtoExemplo.ecommerce?.destaque || false,
        peso: produtoExemplo.ecommerce?.peso || "",
        altura: produtoExemplo.ecommerce?.altura || "",
        largura: produtoExemplo.ecommerce?.largura || "",
        comprimento: produtoExemplo.ecommerce?.comprimento || "",
        tempoPreparacao: produtoExemplo.ecommerce?.tempoPreparacao || "",
        metaTitle: produtoExemplo.ecommerce?.metaTitle || "",
        metaDescription: produtoExemplo.ecommerce?.metaDescription || "",
        keywords: produtoExemplo.ecommerce?.keywords || "",
      },
    },
  })
  ...
  const gerarVariacoes = (atributos: Array<{ nome: string; valores: string[] }>) => {
    ...
    const gerarCombinacoes = (
      ...
      resultado: Array<{ nome: string; codigo: string; estoque?: number }>,
    ) => {
      if (index === atributos.length) {
        ...
        resultado.push({ nome, codigo: "", estoque: 0 })
        return
      }
    ...
    const novasVariacoes: Array<{ nome: string; codigo: string; estoque?: number }> = []
  ...
  // Função para atualizar o estoque de uma variação
  const atualizarEstoqueVariacao = (nome: string, estoque: number) => {
    const novasVariacoes = variacoes.map((v) => (v.nome === nome ? { ...v, estoque } : v))
    setVariacoes(novasVariacoes)
    form.setValue("variacoes", novasVariacoes)
  }
  ...
  // Função para converter entrada de porcentagem
  const converterEntradaPorcentagem = (e: React.ChangeEvent<HTMLInputElement>) => {
    // Remove caracteres não numéricos
    const apenasNumeros = e.target.value.replace(/\D/g, "")

    // Converte para número e divide por 100 para obter o valor em porcentagem
    const valorNumerico = Number.parseFloat(apenasNumeros || "0") / 100
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="details" className="space-y-6 pt-4">
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="text-sm text-muted-foreground">Informações essenciais do produto.</p>
              </div>
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <div
                            className={cn(
                              "flex h-32 w-32 items-center justify-center rounded-md border border-dashed border-input bg-muted/20 transition-colors",
                          ...
                          >
                            {imageSrc ? (
                              ...
                              <div className="flex flex-col items-center justify-center text-muted-foreground">
                                ...
                                <span className="text-xs">{isEditing ? "Clique para adicionar" : "Sem imagem"}</span>
                              </div>
                            )}
                            <input
                              type="file"
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="Digite o nome do produto" {...field} disabled={!isEditing} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="Digite o código do produto" {...field} disabled={!isEditing} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="Digite o NCM" {...field} disabled={!isEditing} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="Digite o GTIN" {...field} disabled={!isEditing} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="Digite o código de barras" {...field} disabled={!isEditing} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                        ...
                        <div className="space-y-2">
                                  ...
                                  <CommandList>
                                    ...
                                    <CommandGroup>
                                      {categorias.map((categoria) => (
                                        <CommandItem
                                          key={categoria.value}
                                          onSelect={() => addCategory(categoria.value)}
                                        >
                                          {categoria.label}
                                        </CommandItem>
                                      ))}
                          ...
                          <div className="flex flex-wrap gap-2">
                            ...
                            {selectedCategories.length === 0 && (
                              <span className="text-sm text-muted-foreground">Nenhuma categoria selecionada</span>
                            )}
                          </div>
                        </div>
                      </FormControl>
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="text-sm text-muted-foreground">Configuração de variações e atributos do produto.</p>
              </div>
                      ...
                      {atributosAdicionados.map((atributo, index) => {
                        ...
                        return (
                          <div key={index} className="rounded-md bg-sidebar p-4 shadow-sm">
                            ...
                            {/* Adicionar valores ao atributo (apenas em modo de edição) */}
                            {isEditing && (
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Variações geradas com base nos atributos e valores selecionados.
                  </p>
                </div>
                    ...
                    <TableBody>
                      {variacoes.map((variacao, index) => (
                        <TableRow key={index}>
                          ...
                          <TableCell>{variacao.estoque !== undefined ? variacao.estoque : 0} unidades</TableCell>
                          <TableCell>
                            <Dialog
                            ...
                            >
                              ...
                              <DialogContent className="sm:max-w-[500px]">
                                ...
                                <DialogFooter>
                                  <Button
                                    type="button"
                                    onClick={() => setOpenEstoqueDialog(false)}
                                    disabled={!isEditing}
                                  >
                                    Salvar
                                  </Button>
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="text-sm text-muted-foreground">Tabelas de preço associadas ao produto e seus valores.</p>
              </div>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          {isEditing && <TableHead className="w-[70px]"></TableHead>}
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                    ...
                    <div className="mx-auto flex max-w-[420px] flex-col items-center justify-center text-center">
                      ...
                      <p className="mb-4 mt-2 text-sm text-muted-foreground">
                        {isEditing
                          ...
                          : "Este produto não possui tabelas de preço associadas."}
                      </p>
                      {isEditing && (
          ...
          {/* Aba de E-commerce */}
          <TabsContent value="ecommerce" className="space-y-8 pt-4">
            {/* Configurações Gerais de E-commerce */}
            <div className="space-y-6">
              <div>
                <h2 className="text-lg font-medium">Configurações de E-commerce</h2>
                <p className="text-sm text-muted-foreground">Configure as opções de e-commerce para este produto.</p>
              </div>
              <div className="space-y-4">
                {/* Status do Produto */}
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <FormField
                    control={form.control}
                    name="ecommerce.ativo"
                    render={({ field }) => (
                      <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4 w-full sm:w-[48%]">
                        <div className="space-y-0.5">
                          <FormLabel className="text-base">Produto Ativo</FormLabel>
                          <FormDescription>Ative para exibir o produto na loja virtual.</FormDescription>
                        </div>
                        <FormControl>
                          <Switch checked={field.value} onCheckedChange={field.onChange} disabled={!isEditing} />
                        </FormControl>
                  ...
                  <FormField
                    control={form.control}
                    name="ecommerce.destaque"
                    render={({ field }) => (
                      <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4 w-full sm:w-[48%]">
                        <div className="space-y-0.5">
                          <FormLabel className="text-base">Produto em Destaque</FormLabel>
                          <FormDescription>Ative para exibir o produto em áreas de destaque.</FormDescription>
                        </div>
                        <FormControl>
                          <Switch checked={field.value} onCheckedChange={field.onChange} disabled={!isEditing} />
                        </FormControl>
                ...
                {/* Dimensões e Peso */}
                <div className="space-y-2">
                  <h3 className="text-sm font-medium">Dimensões e Peso</h3>
                  <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="0.00" {...field} disabled={!isEditing} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      control={form.control}
                      name="ecommerce.altura"
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Altura (cm)</FormLabel>
                          <FormControl>
                            <Input placeholder="0" {...field} disabled={!isEditing} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                ...
                <FormField
                  control={form.control}
                  name="ecommerce.tempoPreparacao"
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Tempo de Preparação (dias úteis)</FormLabel>
                      <FormControl>
                        <Input placeholder="1" {...field} disabled={!isEditing} />
                      </FormControl>
                      <FormDescription>
                        Tempo necessário para preparar o produto para envio após a compra.
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
            ...
            <div className="space-y-6 pt-2">
              <div>
                <h2 className="text-lg font-medium">SEO e Metadados</h2>
                <p className="text-sm text-muted-foreground">
                  Configure informações para otimização em motores de busca.
                </p>
              </div>
              <div className="space-y-4">
                <FormField
                  control={form.control}
                  name="ecommerce.metaTitle"
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Título da Página (Meta Title)</FormLabel>
                      <FormControl>
                        <Input placeholder="Título da página para SEO" {...field} disabled={!isEditing} />
                      </FormControl>
                      <FormDescription>
                ...
                <FormField
                  control={form.control}
                  name="ecommerce.metaDescription"
                  render={({ field }) => (
                    <FormItem>
                ...
                <FormField
                  control={form.control}
                  name="ecommerce.keywords"
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Palavras-chave</FormLabel>
                      <FormControl>
                        <Input
                          placeholder="monitor, ultrawide, 29 polegadas, IPS, 75Hz"
                          {...field}
                          disabled={!isEditing}
                        />
                      </FormControl>
                      <FormDescription>
                        Palavras-chave separadas por vírgula para ajudar na busca do produto.
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
            ...
            <div className="space-y-6 pt-2">
              <div>
                <h2 className="text-lg font-medium">Estatísticas de Vendas</h2>
                <p className="text-sm text-muted-foreground">Desempenho do produto nos últimos 30 dias.</p>
              </div>
              <div className="grid gap-4 md:grid-cols-3">
                <div className="flex flex-col gap-1 rounded-lg border p-3">
                  ...
                  <div className="text-xs text-muted-foreground">+12% em relação ao mês anterior</div>
                </div>
                ...
                <div className="flex flex-col gap-1 rounded-lg border p-3">
                  ...
                  <div className="text-2xl font-semibold">R$ 67.195,80</div>
                ...
                <div className="flex flex-col gap-1 rounded-lg border p-3">
                  ...
                  <div className="text-xs text-muted-foreground">+23% em relação ao mês anterior</div>
                </div>
              </div>
            </div>
          </TabsContent>
